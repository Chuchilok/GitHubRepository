...
//	private Boolean Flagi = false;
	...
	//Integer ii=0;
    //初始化client 和 topic
    public PushMQTTService(final String Serverid) throws MqttException {
    	this.ServerId = Serverid;
    	Map packagesMap = MessageConsumerConfig.readConfig("config.properties");
    	hostName = packagesMap.get("mqttprotocol")+"://" + packagesMap.get("Pmqttserverhost") + ":"
				+ packagesMap.get("Pmqttserverport");
    	mqttserverid = packagesMap.get("Pmqttserverid").toString()+ ServerId;
    	username = packagesMap.get("Pmqttusername").toString();
    	password = packagesMap.get("Pmqttpwd").toString();
...
//						}
						...
						if (msg != null) {
//							System.out.println("MQTTindex"+Serverid+" send data" + count++);
